...
@Component
@Transactional
public class ThongKeDonHangServiceImp implements ThongKeDonHangService{
	...
	public List<Map<String, Object>> thongkeTongDonHangNgay(Date date) {
		...
		for (int i = 0; i < listhd.size(); i++) {
			...
			kq.get(hour).put("value", previous + 1);
		}
	...
	public List<Map<String, Object>> thongkeTongDonHangTuan(Date begin) {
		...
		for (int i = 0; i < listhd.size(); i++) {
			...
			if(day == Calendar.SUNDAY){
				...
				kq.get(6).put("value", previous + 1);
			}else{
				long previous = (long) kq.get(day - 2).get("value");
				kq.get(day - 2).put("value", previous + 1);
			}
	...
	public List<Map<String, Object>> thongkeTongDonHangThang(Date begin) {
		...
		for (int i = 0; i < listhd.size(); i++) {
			...
			kq.get(day - 1).put("value", previous + 1);
	...
	public List<Map<String, Object>> thongkeTongDonHangQuy(Date begin) {
		...
		long sum1 = hoaDonDao.getCount(begin1, begin2);
		record1.put("value", sum1);
		long sum2 = hoaDonDao.getCount(begin2, begin3);
		record2.put("value", sum2);
		long sum3 = hoaDonDao.getCount(begin3, end);
	...
	public List<Map<String, Object>> thongkeTongDonHangNam(Date begin) {
		...
		for(int i = 1; i <= 12 ; i++){
			...
			c.add(Calendar.MONTH, 1);
			Date e = c.getTime();
			record.put("value", hoaDonDao.getCount(b, e));
			kq.add(record);
		}
