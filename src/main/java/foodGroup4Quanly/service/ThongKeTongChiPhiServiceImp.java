...
@Component
@Transactional
public class ThongKeTongChiPhiServiceImp implements ThongKeTongChiPhiService{
	...
	public List<Map<String, Object>> thongkeTongChiPhiNgay(Date date) {
		...
		List<Chiphingay> listcpn = chiPhiNgayDao.getListIn(date, c.getTime());
		List<Luongchonhanvien> listlcnv = luongChoNhanVienDao.getListIn(date, c.getTime());
		List<Tienthuenha> listttn = tienThueNhaDao.getListIn(date, c.getTime());
	...
	public List<Map<String, Object>> thongkeTongChiPhiTuan(Date begin) {
		...
		List<Chiphingay> listcpn = chiPhiNgayDao.getListIn(begin, c.getTime());
		List<Luongchonhanvien> listlcnv = luongChoNhanVienDao.getListIn(begin, c.getTime());
		List<Tienthuenha> listttn = tienThueNhaDao.getListIn(begin, c.getTime());
		for (int i = 0; i < listcpn.size(); i++) {
			c.setTime(listcpn.get(i).getNgay());
	...
	public List<Map<String, Object>> thongkeTongChiPhiQuy(Date begin) {
		...
		for(int i = 1; i <=3; i++){
			...
			Date b = c.getTime();
			c.add(Calendar.MONTH, 1);
			long value = chiPhiNgayDao.getSum(b, c.getTime()) + luongChoNhanVienDao.getSum(b, c.getTime()) + tienThueNhaDao.getSum(b, c.getTime());
